...
def count_and_correlate(val_df, motifs, properties, cluster_name):
    ...
    for m in motifs:
        try:
            esc = re.escape(m)
            df[f"count_{m}"] = df["sequence"].apply(
                lambda s: len(re.findall(f"(?={esc})", str(s))) if pd.notna(s) else 0
            )
            df[f"freq_{m}"] = np.where(df["seq_length"] > 0,
                                         df[f"count_{m}"] / df["seq_length"],
                                         0)
        except re.error as e:
            logger.error(f"[{cluster_name}] Regex error for motif '{m}': {e}. Skipping.")
            # Drop any partial columns
            df.drop(columns=[c for c in df.columns if c.endswith(f"_{m}")], errors='ignore', inplace=True)
            continue
        except Exception as e:
            logger.error(f"[{cluster_name}] Error counting motif '{m}': {e}. Skipping.")
            df.drop(columns=[c for c in df.columns if c.endswith(f"_{m}")], errors='ignore', inplace=True)
            continue

    # Select motifs that were successfully counted
    valid_motifs = [m for m in motifs if f"freq_{m}" in df.columns]
    if not valid_motifs:
    ...
    for m in valid_motifs:
        ...
        for p in properties:
            if p not in df.columns:
                logger.warning(f"[{cluster_name}] Property '{p}' not found. Skipping {m} vs {p}.")
                continue

            x, y = df[freq_col], df[p]
            mask = x.notna() & y.notna()

            if mask.sum() > 2:
                x_m, y_m = x[mask], y[mask]
                # Filter finite and non-zero variance
                if np.all(np.isfinite(x_m)) and np.all(np.isfinite(y_m)) and np.std(x_m) > 0 and np.std(y_m) > 0:
                    try:
                        r, p_r = pearsonr(x_m, y_m)
                        rho, p_s = spearmanr(x_m, y_m)
                        results.append((cluster_name, m, p, r, p_r, rho, p_s, mask.sum()))
                    except Exception as e:
                        logger.warning(f"[{cluster_name}] Correlation error for {m} vs {p}: {e}")
                        results.append((cluster_name, m, p, np.nan, np.nan, np.nan, np.nan, mask.sum()))
                else:
                    logger.warning(f"[{cluster_name}] Skipping {m} vs {p}: insufficient variance or non-finite values.")
                    results.append((cluster_name, m, p, np.nan, np.nan, np.nan, np.nan, mask.sum()))
            else:
                results.append((cluster_name, m, p, np.nan, np.nan, np.nan, np.nan, mask.sum()))
    ...
    if valid_p.any():
        pvals = corr_df.loc[valid_p, "p_pearson"].values
    ...
    logger.info(f"[{cluster_name}] Correlation complete. Found {corr_df['significant_pearson'].sum()} significant after FDR.")
    return corr_df
